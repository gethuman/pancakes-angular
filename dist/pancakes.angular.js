 ...
 * Date: 9/3/14
 *
 * Utilities for generating directives or helping generate them
 */
(function () {
    var dirMap = {
        'href':         'file',
    ...
    function addDirective(directiveName, attrName, filterType, isBind, isBindOnce) {
        app.directive(directiveName, function (i18n, config) {
            return function (scope, element, attrs) {
                var value;

                if (isBind) {
                    var unwatch = scope.$watch(attrs[directiveName], function (value) {
                        if (value) {
                    ...
                    value = filterType === 'file' ?
                        config.staticFileRoot + attrs[directiveName] :
                        i18n.translate(attrs[directiveName], scope);
    ...
    var attrPascal, type;
    for (var attr in dirMap) {
        if (dirMap.hasOwnProperty(attr)) {
