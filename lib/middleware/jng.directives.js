...
function isolateScope(model, scope, attrs) {
    ...
    _.each(scope, function (type, name) {
        ...
        if (type === '=') {
            isolatedModel[name] = me.pancakes.utils.getNestedValue(model, value);
        }
        else if (type === '@') {
...
function getSubviews(subviewFlapjacks) {
    ...
    var jangularDeps = this.getJeffDeps();
    var me = this;
...
function getPartialRenderFn(partial) {
    ...
    return function renderPartial(parentModel, elem, attrs) {
        var model = me.isolateScope(parentModel, partial.scope, attrs);
...
function addDirectivesToJangular(directives) {
    ...
    this.jangular.addDirectives(directiveMap);
    this.jangular.init({
        getTemplate: function(path) {
            return directives[path] ? directives[path] : me.jangular.getTemplate(path);
        }
    });
}
