...
function getGenericDirective(prefix, attrName, filterType, isBind) {
    ...
    return function (scope, element, attrs) {
        ...
        var value = isBind ? me.getNestedValue(scope, originalValue) : originalValue;
        var useSSL, staticFileRoot;

        // if we are using a file filter, add the static file root
        if (filterType === 'file' && config && config.staticFiles) {
        ...
        // else for i18n filter, try to translate
            ...
            value = i18n.translate(value, scope);
        }
