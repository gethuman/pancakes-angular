...
function modifyModel(model, modelFlapjack) {
    ...
    var opts = { dependencies: { model: model }};
    var updatedModel = this.pancakes.cook(modelFlapjack, opts);

    // if model returned, we need to lay it down on top of the original model
    if (updatedModel && updatedModel.toString() !== '[object Promise]') {
        _.extend(model, updatedModel);
    }
}
