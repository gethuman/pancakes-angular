...
function evalModel(currentScope, scopeDefinition, partialModel, partialName) {
    ...
    var user = currentScope && currentScope.user;
    var activeUser = user ? {
        _id: user._id,
        name: user.username,
        role: user.role,
    ...
    var opts = { dependencies: { currentScope: currentScope, activeUser: activeUser }};
    var me = this;
 ...
 * @param appName
 */
function getClientDirectives(appName) {
    var directives = [];
    var me = this;
 ...
 * @param appName
 */
function getExternalClientDirectives() {
    var config = this.pancakes.cook('config', null);
    return config.externalClientDirectives || [];
}
...
/**
 * Add ignored (client-side only) directives to jangular so jangular can strip them when necessary
 * @param directives
 */
function addIgnoredDirectivesToJangular(directives) {
    directives.forEach(function (directiveName) {
        jangular.addIgnoredDirective(directiveName);
    });
}
...
function initDirectives(opts) {
    ...
    _.each(appNames, function (appName) {
        ...
        ignoredDirectives = ignoredDirectives.concat(me.getClientDirectives(appName));
    });

    // external client directives from config...
    //_.extend(ignoredDirectives, me.getExternalClientDirectives());
    ignoredDirectives = ignoredDirectives.concat(me.getExternalClientDirectives());
    ...
    this.addIgnoredDirectivesToJangular(ignoredDirectives);
}
...
module.exports = {
    ...
    getClientDirectives: getClientDirectives,
    getExternalClientDirectives: getExternalClientDirectives,
    ...
    addIgnoredDirectivesToJangular: addIgnoredDirectivesToJangular,
