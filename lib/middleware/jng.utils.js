...
function getNestedValue(data, field, defaultValue) {
    ...
    // needs to be inside so it can reference "data"
    function arrayDereference(o, ref, i) {
        if ( ref ) {
            ref = ref.slice(0, i ? -1 : ref.length);
            if ( ref.indexOf('\'') === 0 || ref.indexOf('\"') === 0 ) { // guessing it's this 'prop' or "prop"
                ref = ref.replace(/'/g, '').replace(/"/g, '');
                return o[ref];
            } else if ( i === 0 ) { // this just isn't always going to work, sigh
                return o[ref];
            } else {
                return o[getNestedValue(data, ref)];
            }
        } else {
            return o;
        }
    ...
    // needs to be inside to reference arrayDereference
    function dotDereference(o, ref) {
        return ( ref.indexOf('[') > -1 ? ref.split('[').reduce(arrayDereference, o) : o[ref] );
    }

    return (!field ? null : field.split('.').reduce(dotDereference, data)) || defaultValue;
}
