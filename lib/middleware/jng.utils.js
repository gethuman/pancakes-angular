...
function getPropnameAndRemains(path) {
    var matches = path.match(jsPropName);
    if (!(matches && matches.length)) {
        throw new Error('Did not match propname: ' + path);
    } else {
        var propName = matches[0];
        return [propName, path.substring(propName.length)];
    }
}

function getClosingBracket(path) {  // sigh... scan for matching, closing bracket, assuming opening one has been removed
    for (var i = 0, depth = 0, len = path.length; i < len; i++) {
    ...
    throw new Error('Could not find closing bracket: ' + path);
}
...
function getNestedValue(originalObj, originalPath, defaultValue) {
    ...
    function chewInnerPath(path) {
        var matches = path.match(jsPropNameQuoted);
        if (matches) {            // QUOTE/QUOTES PROPNAME QUOTE/QUOTES
            return matches[1]; // the first includes the quotes
        } else {                    // PATH
            return chewPath(originalObj, path);
        }
    }

    function chewSubref(path) {
        var firstLetter = path.charAt(0);
        if (firstLetter === '.') {        // DOT PROPNAME
            return getPropnameAndRemains(path.substring(1));
        } else if (firstLetter === '[') { // BOPEN INNERPATH BCLOSE
            path = path.substring(1);
            var idxClose = getClosingBracket(path);
            return [chewInnerPath(path.substring(0, idxClose)), path.substring(idxClose + 1)];
        } else {
            throw new Error('Bad subref: ' + path);
        }
    }

    function chewSubrefs(path) { // should return a propname
        var snr = chewSubref(path);
        if (snr[1]) {         // SUBREF SUBREF
            return [snr[0]].concat(chewSubrefs(snr[1]));
        } else if (snr[0]) {                // SUBREF
            return [snr[0]];
        } else {
            throw new Error('Bad subrefchain: ' + path);
        }
    }

    function chewPath(obj, path) {
        if (!(path && path.length)) {  // EMPTY
            return obj;
        }
        var pnr = getPropnameAndRemains(path);
        var ret = obj[pnr[0]];
    ...
    return chewPath(originalObj, originalPath) || defaultValue;
}
