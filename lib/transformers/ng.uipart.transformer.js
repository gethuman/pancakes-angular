...
_.extend(UIPartTransformer.prototype, pancakes.BaseTransformer.prototype, {
    ...
    transform: function (flapjack, options) {
        ...
        if (this.shouldSkip(uipart)) { return null; }
    ...
    /**
     * If there is no view, then it means there is no component; or if serverOnly or abstract don't do anything
     * @param uipart
     * @returns boolean
     */
    shouldSkip: function shouldSkip(uipart) {
        return !!((!uipart.view && !uipart.subviews && !uipart.parent) ||
            uipart.serverOnly || uipart.abstract);
    },

    /**
     * Replace the first instance of close bracket (i.e. the opening tag) with the directive path
     * also replace the last instance of open bracket with the directive path.
     *
     * It is debatable how useful this is. Turning off for now, but leaving the function
     * in here in case we want to use it in the future.
     *
     * @param html
     * @param filePath
     * @returns {XML|string}
    ...
    /**
     * This is used to convert an object from name -> flapjack to name -> rendered angular fn
