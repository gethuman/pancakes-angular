...
function transform(flapjack, options) {
    ...
    var html = this.getHtml(uipart, options);

    if (options.htmlOnly) {
        return html;
    }

    options.isPage = names.isPage;
...
function getCtrlTemplateModel(uipart, options) {
    ...
    return {
        ...
        rerenderExists:     useRerenderOnEvent || useRerenderOnWatch,
        remodelFnExists:    remodelFnExists && (useRemodelOnEvent || useRemodelOnWatch || !options.isPage),
 ...
 * @returns {string}
 */
function getHtml(uipart, options) {
    options = options || {};
    var me = this;
