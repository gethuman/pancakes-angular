...
function getCtrlTemplateModel(uipart, names, options) {
    ...
    var rebindWatchers = uipart.rebindOnScopeChange ? JSON.stringify(uipart.rebindOnScopeChange): 'null';
    var rebindEvents = uipart.rebindOnEvent ? JSON.stringify(uipart.rebindOnEvent): 'null';
    var rerenderWatchers = uipart.rerenderOnScopeChange ? JSON.stringify(uipart.rerenderOnScopeChange): 'null';
    var rerenderEvents = uipart.rerenderOnEvent ? JSON.stringify(uipart.rerenderOnEvent): 'null';
    var paramInfo = this.getFilteredParamInfo(uipart.controller, options);
    var transOpts = _.extend({ raw: true, isClient: true }, options);
    var useRebindOnWatch = uipart.model && uipart.rebindOnScopeChange && uipart.rebindOnScopeChange.length;
    var useRebindOnEvent = uipart.model && uipart.rebindOnEvent && uipart.rebindOnEvent.length;
    var useRerenderOnWatch = uipart.model && uipart.rerenderOnScopeChange && uipart.rerenderOnScopeChange.length;
    var useRerenderOnEvent = uipart.model && uipart.rerenderOnEvent && uipart.rerenderOnEvent.length;
    ...
    return {
        ...
        rerenderExists:     useRerenderOnEvent || useRebindOnWatch,
        rebindExists:       uipart.model && (useRebindOnWatch || useRebindOnEvent),
        rebindFn:           this.transformers.basic.transform(uipart.model, transOpts),
        rebindWatchers:     rebindWatchers,
        rebindEvents:       rebindEvents,
        rerenderWatchers:   rerenderWatchers,
        rerenderEvents:     rerenderEvents,
        useRebindOnWatch:   useRebindOnWatch,
        useRebindOnEvent:   useRebindOnEvent,
