...
angular.module('pancakesAngular').factory('tplHelper', function ($q, $injector, pageSettings, eventBus) {
     ...
     * @param fn
     */
    function generateRebind(scope, isPartial, fn) {
        if (!fn) { return; }

        // rebinding will call the model function
        scope.rebindModel = function () {
            return $q.when($injector.invoke(fn, null, { model: scope }))
                .then(function (model) {
                    return angular.extend(scope, model);
                });
        };

        // for partials, we want to rebind right away
        if (isPartial) { scope.rebindModel(); }
    }
